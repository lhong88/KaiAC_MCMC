...
#####################################################################
###### MCMC: Populate walkers
#####################################################################
    
pos0 = populate_walkers_from_prior(mod, job_settings, logpost)
np.save(saveto_filename_prefix+'_pos0.npy', pos0)

# calculate and save the log posterior for the initial ensemble
lnprob0= np.array([logpost(walker, auto_rxns_to_fit, 1.0) for walker in pos0]).reshape(len(pos0), 1)
np.save(saveto_filename_prefix+'_pos0_lnprob.npy', lnprob0)

#####################################################################
###### MCMC: Initialization / Annealing
#####################################################################

print("\nInitialization...\n")

best_walker_at_end_of_init = do_initialization(pos0, job_settings, logpost)
