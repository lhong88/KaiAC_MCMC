...
# just to be concise later on in the code, call the total number of 
# estimated parameters dim, since that's the dimension of the space we're
...
auto_rxns_to_fit = auto_rxns #auto_rxns[:1]
...
def neglogpost(q, auto_rxns_to_fit= auto_rxns_to_fit, beta= 1.0):
    '''
...
walkers= np.loadtxt('best_walkers.txt')
walker_ind= int(sys.argv[1])

walker= walkers[walker_ind]

optimized_walker= optimize.minimize(neglogpost, walker, method= 'Powell', options= {'maxiter': 1000})
pickle.dump(optimized_walker, open('output/optimized_walker_' + str(walker_ind) + '.p', 'wb'))
