...
param_names = [
    ...
    'dk_d_ATU', 'dk_p_ATD', 'dk_d_ADS', 'dk_d_ASU', #KaiA/phos on phos/dephos
    'sig' #global error
    ]
...
def converter(q, ATPfrac, Temp):
    ...
    dk_d_ATU, dk_d_ADT, dk_d_ADS, dk_d_ASU = 10**q[:26]
    
    # sparsity hypothesis: single KaiA binding affinity for all KaiC species
    dk_a_UDP= dk_b_UDP= dk_a_SDP= dk_b_SDP= dk_a_TDP= dk_b_TDP= dk_a_STP= dk_b_STP= dk_a_TTP= dk_b_TTP= dk_a_DDP= dk_b_DDP= dk_a_DTP= dk_b_DTP= 1.0
...
# here we do not use any experimental input on KaiA on/off rates
