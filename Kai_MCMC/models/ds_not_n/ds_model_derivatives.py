...
param_names = [
    ...
    'dk_a_T', 'dk_b_T', 'dk_a_D', 'dk_b_D', 'dk_a_S', 'dk_b_S',
    'dp_p_US', 'dk_d_SU', 'dk_p_TD', 'dk_d_DT', 'dk_p_SD', 'dk_d_DS', #phos on phos/dephos
    'dk_d_ATU', 'dk_d_ADT', 'dk_d_ADS', 'dk_d_ASU', #KaiA/phos on phos/dephos
    'sig' #global error
    ]
...
def converter(q, ATPfrac, Temp):
    ...
    dk_d_ATU, dk_d_ADT, dk_d_ADS, dk_d_ASU = 10**q[:32]
    
    # sparsity hypothesis: single KaiA binding affinity for each phosphoform
    dk_a_UDP= 1.0
    dk_b_UDP= 1.0
    dk_a_SDP= dk_a_STP= dk_a_S
    dk_b_SDP= dk_b_STP= dk_b_S
    dk_a_TDP= dk_a_TTP= dk_a_T
    dk_b_TDP= dk_b_TTP= dk_b_T
    dk_a_DDP= dk_a_DTP= dk_a_D
    dk_b_DDP= dk_b_DTP= dk_b_D
    ...
    # detailed balance    
    dk_p_ASD= (dk_b_STP*dk_a_DDP*dk_d_ADS)/(dk_b_DDP*dk_a_STP)
    dk_p_ATD= (dk_b_TTP*dk_a_DDP*dk_d_ADT)/(dk_b_DDP*dk_a_TTP)
    dk_p_AUT= (dk_d_ATU*dk_a_TDP)/dk_b_TDP
    dk_p_AUS= (dk_d_ASU*dk_a_SDP)/dk_b_SDP
...
# note that we don't use any experimental input on KaiA on/off
k_priors= [0., 0., 0., 0., 0., 0.]
dk_priors= [0.]*26
